...
class DetectionModel(paddle.nn.Layer):
    def __init__(self):
        ...
        self.conv2d70 = paddle.nn.Conv2D(out_channels=3, kernel_size=(1, 1), in_channels=64)
        ...
        self.conv2d78 = paddle.nn.Conv2D(out_channels=3, kernel_size=(1, 1), in_channels=64)
        ...
        self.conv2d86 = paddle.nn.Conv2D(out_channels=3, kernel_size=(1, 1), in_channels=64)
    ...
    def forward(self, x0):
        ...
        x1132 = paddle.reshape(x=x970, shape=[1, 67, -1])
        x1134 = paddle.reshape(x=x1050, shape=[1, 67, -1])
        x1136 = paddle.reshape(x=x1130, shape=[1, 67, -1])
        x1137 = [x1132, x1134, x1136]
        x1138 = paddle.concat(x=x1137, axis=2)
        x1140 = paddle.split(x=x1138, num_or_sections=[64, 3], axis=1)
        x1141, x1142 = x1140
...
def main(x0):
    ...
    params = paddle.load(r'/content/runs/detect/train/weights/best_paddle_model/model.pdparams')
